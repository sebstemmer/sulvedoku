from grid import get_coords_in_square, create_all_coords, get_coords_in_square_of_coord, create_coord_to_all_coords_in_row_col_or_square, copy_grid, set_value_in_grid, create_empty_grid, remove_values_from_grid, get_random_empty_where_allowed_values_is_len_1, is_valid, Coord, Cell
from typing import Optional

all_coords: list[Coord] = create_all_coords()
...
def create_all_coords_test() -> None:
    assert all_coords[0] == Coord(row_idx=0, col_idx=0)

    assert all_coords[15] == Coord(row_idx=1, col_idx=6)

    assert len(all_coords) == 81


def get_coords_in_square_test() -> None:
    coords: list[Coord] = get_coords_in_square(square_idx=4)
...
def get_coords_in_square_of_coord_test() -> None:
    coords: list[Coord] = get_coords_in_square_of_coord(
        Coord(
            row_idx=3,
            col_idx=2
        )
...
def create_coord_to_all_coords_in_row_col_or_square_test() -> None:
    assert coord_to_all_coords_in_row_col_or_square[
        Coord(
            row_idx=4,
            col_idx=2
        )
...
def set_value_in_grid_test() -> None:
    ...
    for c in coord_to_all_coords_in_row_col_or_square[first_value_at]:
        if c != first_value_at:
...
get_coords_in_square_test()
get_coords_in_square_of_coord_test()
create_coord_to_all_coords_in_row_col_or_square_test()
copy_grid_test()
